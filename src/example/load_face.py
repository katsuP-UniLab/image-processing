...
def detect_faces_haar(cascade, frame, min_face_size=80):
    ...
    faces = cascade.detectMultiScale(
        gray, scaleFactor=1.1, minNeighbors=8, minSize=(min_face_size, min_face_size)
    )
...
def load_image(image_path):
    """โหลดรูปภาพจากไฟล์ตาม path ที่กำหนด"""
...
def load_dataset_images(dataset_path="dataset"):
    """โหลดรูปภาพทั้งหมดในโฟลเดอร์ dataset คืนค่าเป็น dict ของ person_id -> list of images"""
    dataset = {}
    if not os.path.exists(dataset_path):
        print(f"⚠️ ไม่พบโฟลเดอร์ {dataset_path}")
        return dataset

    for person_folder in os.listdir(dataset_path):
        person_dir = os.path.join(dataset_path, person_folder)
        if os.path.isdir(person_dir):
            images = []
            for file_name in sorted(os.listdir(person_dir)):
                if file_name.lower().endswith((".jpg", ".jpeg", ".png")):
                    img = cv2.imread(os.path.join(person_dir, file_name))
                    if img is not None:
                        images.append(img)
...
def capture_dataset():
    ...
    poses = ["มองตรง", "หันซ้ายนิดหน่อย", "หันขวานิดหน่อย", "เงยหน้าขึ้น", "ก้มหน้าลง"]
    ...
    while count < max_images:
        ...
        for x, y, w, h in faces:
            ...
            cv2.putText(
                display, status, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2
            )
        ...
        cv2.putText(
            display,
            f"Skip blur:{skipped_blur} light:{skipped_light}",
            (10, 90),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
            (150, 150, 150),
